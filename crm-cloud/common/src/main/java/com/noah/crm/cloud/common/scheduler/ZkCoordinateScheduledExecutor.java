...
public class ZkCoordinateScheduledExecutor extends ScheduledThreadPoolExecutor {
    ...
    protected <V> RunnableScheduledFuture<V> decorateTask(Runnable runnable, RunnableScheduledFuture<V> task) {
        ...
        if(!(runnable instanceof DelegatingErrorHandlingRunnable)) {
            throw new EventException("runnable预期类型是DelegatingErrorHandlingRunnable的子类, 实际是: " +
                    runnable.getClass().getName());
        }

        if(!(task instanceof FutureTask)) {
            throw new EventException("task预期类型是FutureTask的子类, 实际是: " + task.getClass().getName());
        }
        ...
        if(delegateRunnableField == null) {
            throw new EventException("DelegatingErrorHandlingRunnable类型中没有找到delegate属性");
        }
        ...
        if(field == null) {
            throw new EventException("FutureTask类型中没有找到callable属性");
        }
