...
public class SignatureUtil {
     ...
     * @return
     */
    public static String signatureWithParamName(Map<String, ?> params,String secretKey, String type) {
        StringBuilder sbuf = new StringBuilder();
        if (null != params) {
            Map<String, Object> treeParam = new TreeMap<>();
            treeParam.putAll(params);
            for (Map.Entry<String, Object> e : treeParam.entrySet()) {
                sbuf.append(e.getKey()).append("=").append(e.getValue().toString()).append("&");
            }
        }
        //把私钥放在最后
        sbuf.append("secret_key").append("=").append(secretKey);
