 function loadDetail( gid){
	 	var detailWin = Ext.getCmp('detail-win');
	 	detailWin.removeAll();
			...
			params:{'entity.gid':gid},
	      	success:function(result){
	    		 ...
	    		 var roleField = new Ext.form.TextField({
	    			 fieldLabel:'角色名',
	    			 value: prizelog.rolename
	    		 });
	    		 if(prizelog.rolename == '' || prizelog.rolename==null){
	    			 roleField.fieldLabel = "<font color=green>角色ID</font>";
	    			 roleField.value=prizelog.roleid;
	    		 }
	    		 ...
	    		 var grid = Ext.getCmp('prize-log-grid');
...
Ext.onReady(function(){
	...
	function PageList(divPrefix){
		...
		this.store = new Ext.data.JsonStore({
			...
			idProperty:'gid',
			remoteSort:true,			
			fields:['gid', 'cid', 'appid','zoneid', 'account','rolename', 'roleid', 'prizeid', 'count', 'priority', 'callback', 'sendStatus', 'processCount',
			        'callbackStatus', 'requestTime','prizeResendCount', 'sendTime', 'callbackHttpStatus' ,'callbackTime', 'callbackCount'],	        
	        url:'/wmeovg/service/prizelog_list.action'
	    });
		...
		var JMSCount = new Ext.Toolbar.TextItem('请求队列:0   回调队列:0');
		...
		this.grid = new Ext.grid.GridPanel({
		    ...
		    tbar:[{
		    	...
		    	handler:function(){
			    	...
			    	handler:function(){
				    	...
				    	var prizelogGids = new Array();
			    		for(var i=0; i <selectedRows.length; i++){
			    			//alert(selectedRows[i].id)
			    			prizelogGids.push(selectedRows[i].id);
			    		}
			    		if(prizelogGids.length > 0){
			    			location.href = "/wmeovg/export/prizeLogSelectedExport.action?prizelogGids=" + prizelogGids;
			    		}
			    	}
			    }, '-', {
		    	text:'导出记录(XLS)',
		    	...
		    	handler:function(){
			    	...
			    	location.href = '/wmeovg/export/prizeLogExport.action?' + instance.searchForm.getForm().getValues(true);
		    	}
		    }, '-', {
		    	text:'导出记录(CSV)',
		    	ref:'../exportCsvBtn',
		    	...
		    	handler:function(){
			    	...
			    	location.href = '/wmeovg/export/prizeLogExportAsCsv.action?' + instance.searchForm.getForm().getValues(true);
		    	}
		    }, '-',JMSCount],
	        ...
	        listeners:{
		    	render:function(grid){
		    		 Ext.TaskMgr.start({
					        run: function(){
						    	Ext.Ajax.request({url:'/wmeovg/service/queueBrowser.action',
			    	            	success:function(result){
	...
	var prizelogList = new PageList('prizelog');
	prizelogList.store.load({params:{start:0, limit:25}});
});