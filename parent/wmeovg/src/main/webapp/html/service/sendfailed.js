 function resend( gid){
		  ...
		  Ext.Ajax.request({url:'/wmeovg/service/queueBrowser_resendByGid.action',
			params:{'gid':gid},
	      	success:function(result){
				switch(parseInt(result.responseText)){
			 		case 0:
			 			Ext.example.msg('提示', '发送请求成功'); break;
			 		case 1:
			 			Ext.example.msg('提示', '发送请求失败，请重试。'); break;
			 		default:
			 			Ext.example.msg('提示', '发送请求失败，请重试。'); break;
			 	}
				var grid = Ext.getCmp('prize-resend-grid');
 ...
 function loadDetail( gid){
	 	var detailWin = Ext.getCmp('prize-resend-detail-win');
	 	detailWin.removeAll();
...
Ext.onReady(function(){
	...
	function PageList(divPrefix){
		...
		this.store = new Ext.data.JsonStore({
			...
			idProperty:'id',
			remoteSort:true,			
			fields:['gid', 'cid', 'appid', 'prizeid' , 'requesttime'],	        
	        url:'/wmeovg/service/queueBrowser_findDlqSendFailedList.action',
	        baseParams: {limit:25}
	    });
		
		function renderResend(value, cellmeta, record, rowIndex, columnIndex, store){
			
			return ("<a href='javascript:void(0)' onclick=\"resend('"+ record.get("gid") +"')\">重新请求</a>");
		}
		...
		this.cm = new Ext.grid.ColumnModel([
		                	              ...
		                	              {header:'原请求时间', dataIndex:'requesttime',width:30},
		                	              {header:'详细', dataIndex:'',renderer:renderDetail,width:10},
		                	              {header:'重新请求', dataIndex:'',renderer:renderResend,width:20}]);
		
		this.grid = new Ext.grid.GridPanel({
			id:'prize-resend-grid',
	...
	var logList = new PageList('sendfailed');
