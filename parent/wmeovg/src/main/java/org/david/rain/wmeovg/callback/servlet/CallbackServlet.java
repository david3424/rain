    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        if (null == callbackService) {
            try {
                ...
                System.err.println("虚拟物品兑换系统回调接口实例化失败：类" + callback + "不存在或不能被实例化");
                throw new ServletException("虚拟物品兑换系统回调接口实例化失败：类" + callback + "不存在或不能被实例化");
            }
            if (!(callbackObject instanceof ICallbackService)) {
                System.err.println("虚拟物品兑换回调接口实例化失败：没有继承ICallbackService接口");
                throw new ServletException(
                        "虚拟物品兑换回调接口没有继承callback.servlet.ICallbackService接口");
            }
