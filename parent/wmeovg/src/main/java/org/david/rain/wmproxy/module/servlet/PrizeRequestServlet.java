...
public class PrizeRequestServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		clientInfoMng = (ClientInfoMng) ctx.getBean(CLIENTINFO_BEAN_NAME,
				ClientInfoMng.class);
		whiteListMng = (WhiteListMng) ctx.getBean(WHITELIST_BEAN_NAME,
				WhiteListMng.class);
		requestMessageProducer = (RequestMessageProducer) ctx
				.getBean(REQUESTMESSAGEPRODUCER_BEAN_NAME, RequestMessageProducer.class);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			...
			ex.printStackTrace();
			log.error("流水号[{}]：角色名和callback解码错误；{}", prizeLog.getGid(),
		...
		// 验证请求合法性
		try {
			if (!SignatureUtil.isPrizeLegal(prizeLog, req.getParameter("sign"),
					privateKey)) {
