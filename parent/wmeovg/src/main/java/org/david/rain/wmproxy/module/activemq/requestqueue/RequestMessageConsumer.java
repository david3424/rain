...
//        log.error("[{}] 222 进入请求队列 receive  时间：[{}]", prizeLog.getGid(), System.currentTimeMillis());
        ...
        if (null == prizeSendType) {
            prizeSendType = PrizeSendType.ROLENAME;
        }
        ...
        String rolename = prizeLog.getRolename();
        ...
        try {
            ...
            log.error("读取白名单状态错误", e.getMessage());
            return;
        }

        if (whiteListStatus == 0) {
            if (count == 1 && prizeLog.getSendStatus() != 0) {
...
//                    log.error("[{}] 222 333 receive 处理中发送失败 change status begin 时间：[{}]", prizeLog.getGid(), System.currentTimeMillis());
                    prizeLogMng.accumulatePrizeResendCountByGID(gid);
//                    log.error("[{}] 222 444 receive 处理中发送失败 change status end 时间：[{}]", prizeLog.getGid(), System.currentTimeMillis());
                    try {
                    ...
                    if (resendCount % 3 == 0) {
                        ...
                        throw new FailToSendPrizeException();
                    }
                }
            } else if (count > 1) {
...
//        log.error("[{}] 333 进入请求队列 receive 请求成功，进入回调 开始 时间：[{}]", prizeLog.getGid(), System.currentTimeMillis());
        process(prizeLog, status, processCount);
//        log.error("[{}] 444 进入请求队列 receive 请求成功， 进入回调 开始 时间：[{}]", prizeLog.getGid(), System.currentTimeMillis());
    }
