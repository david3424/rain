 ...
 */
public interface ServerRemindMapper {
    @Insert("insert into monitor_remind (item_id,remind_type,user_id,create_time,status) " +
            "values (#{itemId},#{remindType},#{userId},#{createTime},#{status})")
    public void insertRemind(RemindInfo remindInfo);


    @Update("update monitor_remind set remind_type = #{remindType} where id=#{id}")
    public int updateRemind(RemindInfo remindInfo);

    @Delete("delete from monitor_remind where id = #{id}")
    ...
    @Select("select a.id,a.item_id as itemId, a.remind_type as remindType, b.ch_name as chName, b.phone, b.email" +
            ", a.user_id as userId from monitor_remind a, monitor_user b where a.user_id =  b.id and a.item_id = #{itemId}")
