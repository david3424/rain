...
<div class="log">
    <c:choose>
        ...
        <c:otherwise>
            <p>欢迎您，${sessionScope.USER.account}！  USERID：${userid}<a hidefocus="true" class="logout" href="javascript:void(0)"
                                                   id="logout">登出</a></p>
        </c:otherwise>
    </c:choose>
</div>
...
<script type="text/javascript" src="${ctx}/extend/dota2passport/dota2.passport.js"></script>
<script type="text/javascript">

    var username = "${sessionScope.USER.account}";
    function login() {
        dota2.passport.islogin({session: 'USER', _false: function () {
            dota2.passport.login({session: 'USER',hdid: "world2_login_status"});
        }});
    }
    function logout() {
        dota2.passport.logout({session: 'USER'});
    }
